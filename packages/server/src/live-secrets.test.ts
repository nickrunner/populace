...
import { ensureSimulation, resolveSimulationConfig, setCohortSize, snapshotConfig, type ProcessConfig } from "./config-store.js";
...
describe("connecting to the target from a run's frozen config", () => {
  ...
  it("refuses to connect at all when the live credentials cannot be restored", async () => {
    ...
    try {
      ...
      await setCohortSize(server.store, "default", persona!, 0);
      target.seen.length = 0;
