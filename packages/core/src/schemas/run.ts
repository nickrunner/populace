...
/**
 * `pending` is a run whose row exists but whose daemon has not ticked yet; `paused` is a drain
 * that finished. A run that the process lost track of (the daemon died, `serve` was killed
 * mid-run) is reconciled to `failed` on the next open rather than left looking alive — that
 * distinction is the whole reason the table exists (ADR-0024).
 */
...
export const RunSchema = z.object({
  ...
  /** The authored target this run was started against. Empty for a run that predates the table. */
